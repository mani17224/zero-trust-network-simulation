...
class JSONFormatter(logging.Formatter):
    def format(self, record):
        entry = {"timestamp": datetime.now(timezone.utc).isoformat(),
                 "level": record.levelname, "service": "service-b", "message": record.getMessage()}
...
REQUEST_LATENCY = Histogram("service_b_request_duration_seconds", "Latency",     ["endpoint"])


@asynccontextmanager
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    logger.info("service-b starting", extra={"port": 8002})
    yield
    logger.info("service-b shutting down")


app = FastAPI(title="Zero Trust service-b", version=os.getenv("APP_VERSION","1.0.0"),
...
app.include_router(router, tags=["service-b"])


@app.get("/health")
@app.get("/healthz")
async def health():
    return {"status":"healthy","service":"service-b","version":os.getenv("APP_VERSION","1.0.0")}


@app.get("/ready")
async def ready():
    return {"status":"ready","service":"service-b"}
